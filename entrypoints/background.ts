<template>
  <div class="bg-gray-50 p-3 w-120">
    <div class="max-w-md mx-auto">
          ...
          <div class="flex items-center gap-4">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-baseline gap-2 mb-1">
                <h1 class="text-2xl font-bold text-gray-900 tracking-tight">GTM Helper</h1>
              </div>
            </div>
          </div>
        </div>
      ...
      <!-- Settings List with Fixed Borders -->
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-visible">
        <div class="divide-y divide-gray-100">
          <div 
            v-for="(feature, key) in settings.features" 
            :key="key" 
            :class="[
              'flex items-center justify-between p-4 relative transition-colors duration-200',
              feature.enabled ? 'bg-orange-50/50' : 'bg-white'
            ]"
          >
            <div class="flex items-center gap-3 flex-1">
              <div class="flex flex-col">
                ...
                <div class="flex gap-1 mt-1">
                  <span 
                    v-for="platform in feature.activableOn" 
                    :key="platform"
                    :class="[
              ...
              >
                <div class="w-4 h-4 rounded-full bg-blue-50 hover:bg-blue-100 border border-blue-200 flex items-center justify-center cursor-help transition-colors duration-200">
                  <span class="text-xs font-semibold text-blue-600">i</span>
                </div>
              </div>
            </div>

            <!-- Cool Modern Toggle Switch -->
            <div class="ml-4">
              <button @click="toggleSetting(key)" class="relative outline-none" type="button">
                <div :class="[
                  'w-12 h-6 rounded-full transition-colors duration-300 ease-in-out',
                  feature.enabled
                    ? 'bg-gradient-to-r from-orange-400 to-orange-500 shadow-inner shadow-orange-600/20'
      ...
      <div 
        v-if="activeTooltip"
        class="fixed z-50 w-72 p-3 bg-white rounded-lg shadow-lg border border-gray-200"
        :style="tooltipStyle"
        style="animation: tooltipFadeIn 0.2s ease-out forwards;"
      >
        <p class="text-sm text-gray-700 leading-relaxed">{{ settings.features[activeTooltip]?.description }}</p>
      </div>
      ...
      <div class="mt-4">
        <button 
          ...
          class="cursor-pointer w-full flex items-center justify-center gap-2 px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm text-gray-600 hover:text-gray-900 hover:bg-gray-50 hover:border-gray-300 transition-all duration-200 shadow-sm"
        >
...
const settings = reactive({
  features: {
    ...
    entitiesFilteringPreview: {
      description: 'Option to filter Tags and variables on preview mode',
      enabled: true,
      activableOn: ["GTMTA", "GTMTASS"]
    },
    entitiesFiltering: {
      description: 'Option to filter Tags and variables on UI',
      enabled: true,
      activableOn: ["GTMUI"]
    }
  }
...
const getPlatformColor = (platform) => {
  const colors = {
    GTMUI: 'bg-blue-100 text-blue-700',
    GTMTA: 'bg-green-100 text-green-700',
    GTMTASS: 'bg-purple-100 text-purple-700'
  }
